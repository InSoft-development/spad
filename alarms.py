...
client = clickhouse_connect.get_client(host='localhost', username='default', password='asdf')
...
for g in groups:
    header_p += "anomaly_time"+str(g)+" Nullable(Float64)," \
        "anomaly_date"+str(g)+" Nullable(DateTime64)," \
        "P"+str(g)+" Float64, "
...
while not terminate_flag:
    ...
    alarms = pd.DataFrame()
    for g in groups:
        lstm = client.query_df("SELECT target_value,count, timestamp FROM lstm_group"+str(g)+" order by timestamp desc limit 1")
        print("LSTM:\n", lstm)
        ...
        P = max(lstm["target_value"][0], pot["probability"][0])
